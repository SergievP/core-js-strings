...
function sumOfCodes(str) {
  ...
  return [...str].reduce((sum, char) => sum + char.codePointAt(0), 0);
}
...
function isPalindrome(str) {
  const formatedStr = str
    .replace(/[ \t!"#$%&'()*+,-./:;<=>?@[\\\]^_`{|}~]/g, '')
    .toLowerCase();
  const lengthOfStr = formatedStr.length;
  const center = lengthOfStr % 2 === 0 ? lengthOfStr / 2 : lengthOfStr / 2 + 1;
  const secondPart = formatedStr.slice(center).split('').reverse().join('');
  const firstPart = formatedStr.slice(0, center);

  return firstPart.startsWith(secondPart);
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const codeChar = str.codePointAt(i);

    if ((codeChar > 64 && codeChar < 78) || (codeChar > 96 && codeChar < 110)) {
      newArr.push(String.fromCodePoint(codeChar + 13));
    } else if (
      (codeChar > 77 && codeChar < 91) ||
      (codeChar > 109 && codeChar < 124)
    ) {
      newArr.push(String.fromCodePoint(codeChar - 13));
    } else {
      newArr.push(str[i]);
...
function getCardId(value) {
  ...
  const cardSuit = value[value.length - 1].codePointAt();
  const cardId = arr.indexOf(value[0]);
